...
static void tft_draw_char(char c, uint8_t x, uint8_t y, uint16_t color,
                          uint8_t size) {
  GFXglyph *glyph = font->glyph + c - font->first;
  uint16_t bitmap_offset = glyph->bitmapOffset;

  // uint8_t height = font->yAdvance - 1;
  // uint8_t baseline = y + height - 2;
  // uint8_t y0 = baseline + glyph->yOffset;
...
static void tft_draw_text(char *text, uint8_t x, uint8_t y, uint16_t color,
                          uint8_t size) {
  uint8_t offset = 0;
  for (; *text; text++) {
...
int main() {
  ...
  tft_draw_text("IT'S", 0, 0, TFT_COLOR_GREEN, 3);
  tft_draw_text("HACK", 0, 30, TFT_COLOR_GREEN, 3);
  tft_draw_text("O'", 0, 60, TFT_COLOR_GREEN, 3);
  tft_draw_text("CLOCK", 0, 90, TFT_COLOR_GREEN, 3);
  tft_draw_text("!", 0, 120, TFT_COLOR_GREEN, 3);

  tft_draw_text("rbaron", 0, 140, TFT_COLOR_RED, 2);
  tft_draw_text(".net", 0, 150, TFT_COLOR_RED, 2);
